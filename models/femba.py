...
class MambaClassifier(nn.Module):
    ...
    def __init__(
        self, embed_dim, grid_size, num_classes, num_channels, classification_type: str
    ):
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.unflatten = nn.Unflatten(dim=1, unflattened_size=(embed_dim, grid_size[0]))

        self.conv_transpose = nn.ConvTranspose2d(
            in_channels=embed_dim, out_channels=1, kernel_size=patch_size, stride=stride
        )
...
class FEMBA(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, grid_size[1], grid_size[0] * self.embed_dim)
        )
        ...
        if num_classes == 0:
            ...
            self.classifier = MambaClassifier(
                embed_dim, grid_size, num_classes, num_channels, classification_type
            )
...
def load_pretrained_femba(model_path, device="cuda"):
    """Load pretrained FEMBA model from safetensors."""
    from safetensors.torch import load_file

    state_dict = load_file(model_path)
    # Compatible with huggingface weights, head not trained
    femba = FEMBA(
        seq_length=1280,
        num_channels=22,
        num_classes=2,
    ...
    filtered_state_dict = {
        k: v for k, v in state_dict.items() if k in femba.state_dict()
    }
    femba.load_state_dict(filtered_state_dict, strict=False)
    femba.eval()
    femba.to(device)
